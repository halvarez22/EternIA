...
export default function App() {
  ...
  const [theme, setTheme] = useState<'dark' | 'light'>(() => {
    try {
      const savedTheme = localStorage.getItem('eternia_theme_v10');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('eternia_theme_v10', theme);
      if (theme === 'light') {
  ...
  const [stories, setStories] = useState<FinishedStory[]>(() => {
    try {
      const saved = localStorage.getItem('eternia_stories_v10');
      if (saved) {
  ...
  const [featuredStoryId, setFeaturedStoryId] = useState<string>(() => {
    try {
      const savedFeatured = localStorage.getItem('eternia_featured_story_id_v10');
  ...
  const [isAdminLoggedIn, setIsAdminLoggedIn] = useState<boolean>(() => {
    try {
      return localStorage.getItem('eternia_admin_logged_in_v10') === 'true';
    } catch (e) {
      return false;
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('eternia_stories_v10', JSON.stringify(stories));
    } catch (e) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('eternia_featured_story_id_v10', featuredStoryId);
    } catch (e) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('eternia_admin_logged_in_v10', isAdminLoggedIn ? 'true' : 'false');
    } catch (e) {

    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 relative z-10">
        ...
        <div className="flex flex-wrap items-center gap-2.5 mb-6">
          ...
          <button
            onClick={() => setSelectedServiceType('video_musicalizado')}
            className={`px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 ${
              selectedServiceType === 'video_musicalizado'
                ? 'bg-gradient-to-r from-amber-400 to-pink-400 text-black font-bold shadow-md'
                : 'glass text-[var(--text-secondary)] hover:text-white border border-[var(--glass-border)]'
            }`}
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span>Videos Musicalizados ({stories.filter(s => s.serviceType === 'video_musicalizado').length})</span>
          </button>
        </div>
